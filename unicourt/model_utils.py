    ...
    def set_attribute(self, name, value):
        ...
        if get_simple_class(name) != str:
            error_msg = type_error_message(
                var_name=name, var_value=name, valid_classes=(
                    str,), key_type=True
            )
            raise ApiTypeError(
                error_msg,
        ...
        if (name,) in self.validations:
            check_validations(self.validations, (name,),
...
COERCION_INDEX_BY_TYPE = {
    ...
    date: 10,
    str: 11,
    # 'file_type' is an alias for the built-in 'file' or 'io.IOBase' type.
    file_type: 12,
}
...
def order_response_types(required_types):
    ...
    sorted_types = sorted(
        required_types, key=lambda class_or_instance: index_getter(
            class_or_instance)
    )
    return sorted_types
...
def change_keys_js_to_python(input_dict, model_class):
    ...
    if getattr(model_class, "attribute_map", None) is None:
        return input_dict
    output_dict = {}
    reversed_attr_map = {value: key for key,
                         value in model_class.attribute_map.items()}
...
def is_valid_type(input_class_simple, valid_classes):
    ...
    if not valid_type and (
        issubclass(input_class_simple,
                   OpenApiModel) or input_class_simple is none_type
    ):
        for valid_class in valid_classes:
            ...
            if not (
                issubclass(
                    valid_class, OpenApiModel) and valid_class.discriminator
            ):
                continue
            ...
            valid_type = is_valid_type(
                input_class_simple, discriminator_classes)
            if valid_type:
                return True
    return valid_type
...
def validate_get_composed_info(constant_args, model_args, self):
    ...
    oneof_instance = get_oneof_instance(
        self.__class__, model_args, constant_args)
